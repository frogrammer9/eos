...
void handle_PCINT(pin_group_t pg) {
	...
	for(u8 i = 0; i < 8; ++i) {
		...
		if((state_change & mask) && ((u8[2]){pin_input_falling[pg], pin_input_rising[pg]}[(state & mask) != 0]) & mask) _eos_push_event(event);
	}
}
...
void eos_config_pin_input (pin_t pin, bool pullup, pin_input_edge_t edge) {
	free_pin(pin);
	if(pin_reserved[pin.group] & pin.number) { eos_indicate_error(ERR_PIN_IS_BUSY); return; }
	if((edge & 0b01) == 0) pin_input_falling[pin.group] |= pin.number;
	if((edge & 0b10) == 0) pin_input_rising[pin.group] |= pin.number;
	switch(pin.group) {
		case PG_B: DDRB &= ~pin.number; if(pullup) PORTB |= pin.number; PCMSK0 |= pin.number; break;
		case PG_C: DDRC &= ~pin.number; if(pullup) PORTC |= pin.number; PCMSK1 |= pin.number; break;
		case PG_D: DDRD &= ~pin.number; if(pullup) PORTD |= pin.number; PCMSK2 |= pin.number; break;
	}
}
...
void eos_set_pin_state(pin_t pin, bool state) {
	...
	switch(pin.group) {
        case PG_B: if(state) PORTB |= pin.number; else PORTB &= ~pin.number; break;
        case PG_C: if(state) PORTC |= pin.number; else PORTC &= ~pin.number; break;
        case PG_D: if(state) PORTD |= pin.number; else PORTD &= ~pin.number; break;
	}
}
